...
func (this *DNSLaProvider) GetRecords(domain string) (records []*dnstypes.Record, err error) {
	...
	for i := 1; i < 5000; i++ {
		...
		for _, rawRecord := range resp.Data.Results {
			var recordType = this.recordTypeNameFromString(rawRecord.Type)
...
func (this *DNSLaProvider) QueryRecord(domain string, name string, recordType dnstypes.RecordType) (*dnstypes.Record, error) {
	...
	for _, rawRecord := range resp.Data.Results {
		var recordTypeName = this.recordTypeNameFromString(rawRecord.Type)
...
// recordTypeNameFromString 将DNSLa的字符串类型代码转换为标准记录类型名称
func (this *DNSLaProvider) recordTypeNameFromString(typeStr string) string {
	switch typeStr {
	case "1":
		return "A"
	case "2":
